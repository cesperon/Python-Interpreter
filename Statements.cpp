...
AssignmentStatement::AssignmentStatement(std::string lhsVar, std::unique_ptr<ExprNode> Index, std::unique_ptr<ExprNode> rhsExpr):
     _lhsVariable{std::move(lhsVar)}, _index{std::move(Index)}, _rhsExpression2{std::move(rhsExpr)} {}
/*
AssignmentStatement::AssignmentStatement(std::string lhsVar):
...
void AssignmentStatement::evaluate(SymTab &symTab) {
    ...
    else if(rhsExpression2() != nullptr){
        
        if(symTab.getValueFor(lhsVariable())->type() == StringArray::STRINGARRAY){
            if(rhsExpression2()->evaluate(symTab)->type() != StringDescriptor::STRING){
                std::cout<<"array value not of compatible types"<<std::endl;
                exit(1);
            }
            auto Idesc = dynamic_cast<NumberDescriptor*>(_index->evaluate(symTab).get());
            int index = Idesc->value.intValue;
            
            auto desc = dynamic_cast<StringDescriptor*>(_rhsExpression2->evaluate(symTab).get());
            std::string Val = desc->value;
            dynamic_cast<StringArray*>(symTab.getValueFor(lhsVariable()).get())->setSubStr(index, Val );
        }
        else if(symTab.getValueFor(lhsVariable())->type() == NumberArray::NUMBERARRAY){
            if(rhsExpression2()->evaluate(symTab)->type() != NumberDescriptor::INTEGER){
                std::cout<<"array value not of compatible types"<<std::endl;
                exit(1);
            }
            auto Idesc = dynamic_cast<NumberDescriptor*>(_index->evaluate(symTab).get());
            ...
            auto desc = dynamic_cast<NumberDescriptor*>(_rhsExpression2->evaluate(symTab).get());
            int Val = desc->value.intValue;
            dynamic_cast<NumberArray*>(symTab.getValueFor(lhsVariable()).get())->setSubNum(index, Val );
        }
...
std::unique_ptr<ExprNode>&AssignmentStatement::rhsExpression2() {
    return _rhsExpression2;
}
