...
public:
    ...
    AssignmentStatement(std::string lhsVar, std::unique_ptr<ExprNode> subExpr, std::unique_ptr<ExprNode> rhsExpr);
    AssignmentStatement(std::string lhsVar);
    std::string &lhsVariable();
	std::unique_ptr<ExprNode> &rhsExpression();
    std::unique_ptr<ExprNode> &rhsExpression2();
    int arrSize();
...
private:
    ...
    std::unique_ptr<ExprNode> _rhsExpression2;
   // int _index;
    std::unique_ptr<ExprNode> _index;
};
